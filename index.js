    ...
    wss.on('connection', (ws, req) => {
        ...
        async function executeCommand(ws, container, command) {
            try {
                ...
                stream.on('end', () => {
                    ws.send('\u001b[1m\u001b[33mcontainer@coderxsa~ \u001b[0mServer marked as Offline\r\n');
                });
        ...
        async function performPowerAction(ws, container, action) {
            ...
            if (!actionMap[action]) {
                ws.send(`\r\n\u001b[33m[coderxsa] \x1b[0Invalid action: ${action}\r\n`);
                return;
            }
        
            ws.send(`\r\n\u001b[33m[coderxsa] \x1b[0mWorking on ${action}...\r\n`);
        
            try {
                await actionMap[action]();
            } catch (err) {
                //console.error(`Error performing ${action} action:`, err);
                ws.send(`\r\n\u001b[33m[coderxsa] \x1b[0Action failed: ${err.message}\r\n`);
            }
        }
...
app.get('/', async (req, res) => {
    try {
        ...
        const response = {
            versionFamily: 1,
            versionRelease: 'coderxsa ' + config.version,
            online: true,
        ...
        res.status(500).json({ error: 'Docker is not running - coderxsa will not function properly.' });
    }
});
